...
export class TowerEventsController extends BaseController {
  constructor() {
    ...
    this.router
      ...
      .delete("/:eventId/delete", this.deleteEvent);
  }
  ...
  async getEventById(req, res, next) {
    try {
      const towerEvent = await towerEventsService.getEventById(
        req.params.eventId
      );
      res.send(towerEvent);
    } catch (error) {
  ...
  async editEvent(req, res, next) {
    try {
      let eventData = req.body;
      const towerEvent = await towerEventsService.editEvent(
        req.params.eventId,
        eventData,
        req.userInfo.id
      );
  ...
  async cancelEvent(req, res, next) {
    try {
      const towerEvent = await towerEventsService.cancelEvent(
        req.params.eventId,
        req.userInfo.id
      );
      res.send(towerEvent);
  ...
  async deleteEvent(req, res, next) {
    try {
      const towerEvent = await towerEventsService.deleteEvent(
        req.params.eventId,
        req.userInfo.id
      );
      res.send(towerEvent);
