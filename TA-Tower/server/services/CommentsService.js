...
class CommentsService {
  ...
  async getCommentById(commentId) {
    let comment = await dbContext.Comment.findById(commentId).populate(
      "creator",
      "name picture"
    );
    if (!comment) {
  ...
  async getCommentsByEventId(eventId) {
    let comments = await dbContext.Comment.find({ eventId }).populate(
      "creator",
      "name picture"
    );
    return comments;
  ...
  async createComment(commentData) {
    ...
    if (towerEvent.isCanceled == true) {
      throw new BadRequest(
        "This event has been cancelled, you can no longer leave a comment."
      );
    }
  ...
  async deleteComment(commentId, userInfo) {
    ...
    if (comment.creatorId != userInfo) {
      throw new Forbidden(
        "You cannot delete a comment that you did not create."
      );
    }
