...
class TowerEventsService {
  async getAllEvents() {
    let towerEvents = await dbContext.Event.find().populate(
      "creator",
      "name picture"
    );
    return towerEvents;
  ...
  async getEventById(eventId) {
    let towerEvent = await dbContext.Event.findById(eventId).populate(
      "creator",
      "name picture"
    );
    if (!towerEvent) {
  ...
  async editEvent(eventId, eventData, userInfo) {
    ...
    if (towerEvent.isCanceled == true) {
      throw new Forbidden(
        `${eventId} has been cancelled. You cannot make changes to cancelled events.`
      );
    }
  ...
  async cancelEvent(eventId, userInfo) {
    ...
    if (towerEvent.creatorId != userInfo) {
      throw new Forbidden(
        "You cannot cancel an event that you did not create."
      );
    }
  ...
  async deleteEvent(eventId, id) {
    ...
    if (event.creatorId != id) {
      throw new Forbidden(
        "You can't delete this event, you're not the creator of it."
      );
    }
    await event.remove();
  }
